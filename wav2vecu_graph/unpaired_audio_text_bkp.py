    ...
    def valid_step(self, sample, model, criterion):
        ...
        for i, (x, t, id, bin_label, gt_bin_label, bin_score) in enumerate(
            zip(
                ...
                sample["id"],
                sample["net_input"]["bin_labels"] if "bin_labels" in sample["net_input"] else [None] * len(z),
                sample["net_input"]["gt_bin_labels"] if "gt_bin_labels" in sample["net_input"] else [None] * len(z),
                bin_scores,
            )
        ):  
            if t is not None:
            ...
            if bin_label is not None:
                y = bin_label.nonzero().squeeze(-1).cpu().numpy()
