...
def process_segment_predictions(
    ...
    r_dup_count = 0
    labels = []
    for i, (hypo, ref) in enumerate(zip(hypos, refs)):
        hyp_segs = hypo.cumsum(-1).cpu().tolist()
        hyp_segs = list(map(str, hyp_segs))
...
def prepare_result_files(cfg: UnsupGenerateConfig):
    ...
    return {
        ...
        "hypo.segments": get_res_file("", "src"),
        "ref.segments": get_res_file("_gt", "src"),
    }
    ...
    with progress_bar.build_progress_bar(cfg.fairseq.common, itr) as t:
        for i, sample in enumerate(t):
            ...
            if hypo_futures is not None:
                ...
                bin_scores = res["bin_scores"]
                hypo_segments = (bin_scores > 0).long()
                ref_segments = res["gt_bin_labels"].long()
...
def main(cfg: UnsupGenerateConfig, model=None):
    ...
    if gen_result.b_len > 0:
        ...
        logger.info(f"Boundary recall (harsh): {boundary_recall_harsh}")
        logger.info(f"Boundary recall (lenient): {boundary_recall_lenient}")
